...
def resp(wfn, options={}):
    ...
    charges = scipy.optimize.minimize(resp_objective, options['charges'], args=(None, options), method='SLSQP', tol=1e-8, options={'ftol':1e-8},
              constraints={'type': 'eq', 'fun':resp_constraint, 'args':[None, options['mol_charge'], options['CHARGE_GROUPS'], False]})
    charges = charges.x
    if options['TWO_STAGE_FIT']:
        ...
        charges2 = scipy.optimize.minimize(resp_objective, charges[options['FIT2']], args=(None, options, True), method='SLSQP', tol=1e-8, options={'ftol':1e-8},
                   constraints={'type': 'eq', 'fun':resp_constraint, 'args':[None, options['mol_charge'], options['CHARGE_GROUPS'], True, options['charges'], options['FIT2']]})
    ...
    if options['CHARGE_GROUPS'] is not None:
